...
async function loadFullDictionary() {
    try {
        ...
        const allowedGuessesUrl = "https://gist.githubusercontent.com/cfreshman/cdcdf777450c5b5301e439061d29694c/raw/wordle-allowed-guesses.txt";

        // Fetch both files simultaneously
        const [ansRes, allowRes] = await Promise.all([fetch(answersUrl), fetch(allowedGuessesUrl)]);
        const ansText = await ansRes.text();
        const allowText = await allowRes.text();

        // Process the core secret answers
        SECRET_ANSWER_LIST = ansText.split('\n')
                                    .map(w => w.trim().toUpperCase())
                                    .filter(w => w.length === 5);

        // Process the allowed input guesses
        const allowedList = allowText.split('\n')
                                     .map(w => w.trim().toUpperCase())
                                     .filter(w => w.length === 5);
        ...
        console.log(`📚 Success! Unlocked ${SECRET_ANSWER_LIST.length} puzzle answers and ${REJECT_DICTIONARY.length} total valid guess words.`);
    } catch (error) {
        console.error("❌ Failed to fetch vocabulary streams, falling back:", error);
...
io.on('connection', (socket) => {
    ...
    socket.on('create_room', () => {
        ...
        const randomSecretWord = SECRET_ANSWER_LIST[Math.floor(Math.random() * SECRET_ANSWER_LIST.length)] || "HARES";
        ...
        console.log(`🆕 Room ${roomCode} created. Secret Word is: ${randomSecretWord}`);
    });
    ...
    socket.on('submit_guess', (data) => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (secretLetterCounts[guessLetter] && secretLetterCounts[guessLetter] > 0) {
                result[i] = 'present';
        ...
        if (isWinningGuess) {
            ...
            setTimeout(() => {
                ...
                console.log(`🧹 Cleaned up and deleted Room ${roomCode} from active memory.`);
            }, 6000);
        }
    });
